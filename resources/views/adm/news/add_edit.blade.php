        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header">
                    <button type="button" class="btn btn-sm btn-outline-danger" onClick="location.href='{{ route('admin.news.index') }}'">{{ __('Cancel') }}</button>
                ...
                @if ($isEdit)
                    <form class="card-body" action="{{ route('admin.news.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    @method('PUT')
                @else
                    <form class="card-body" action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data">
                @endif
                    @csrf
